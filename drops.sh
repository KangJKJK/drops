...
echo -e "${GREEN}출처:https://github.com/Galkurta/Drops-BOT${NC}"

echo -e "${GREEN}설치 옵션을 선택하세요:${NC}"
echo -e "${YELLOW}1. Drops 봇 새로 설치${NC}"
echo -e "${YELLOW}2. 재실행하기${NC}"
read -p "선택: " choice

case $choice in
  1)
    echo -e "${GREEN}Drops 봇을 새로 설치합니다.${NC}"
    ...
    git clone https://github.com/Galkurta/Drops-BOT.git
    cd "$WORK"
    ...
    read -p "애플리케이션-세션저장소-Drops와 관련된 URL클릭 후 나오는 UserID나 QueryID를 적어두세요 (엔터) : "
  ...
  2)
    echo -e "${GREEN}Drops 봇을 재실행합니다.${NC}"
